...
import { Sort, VariantSizeOrder } from '../../../interfaces';

interface Props {
  sort: Sort;
  order: VariantSizeOrder;
  setSort: Dispatch<SetStateAction<Sort>>;
  setOrder: Dispatch<SetStateAction<VariantSizeOrder>>;
}

const VariantSizeSelct: FC<Props> = (props) => {
  ...
  const onChangeOrder = (e: SelectChangeEvent) => {
    setOrder(e.target.value as VariantSizeOrder);
  };
    ...
    <>
      ...
      <FormControl sx={{ m: 1, minWidth: 120 }}>
        ...
        <Select value={order} label="Ordenar por" onChange={onChangeOrder}>
          ...
          <MenuItem value="short">Abreviatura</MenuItem>
        </Select>
      </FormControl>
    </>
  );
...
export default VariantSizeSelct;
